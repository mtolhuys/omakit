...
one guard every write is held to: `omakit lab setup` prepares the newest
Omarchy release, fetches bytes after one consent that names the exact size
and destination (`--yes` for an agent), verifies them against the digest
the release publishes and the pinned signer's signature before anything
boots them, and builds one base; `omakit lab prove` boots a guest
