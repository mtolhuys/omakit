// `omakit completion bash|zsh|fish`: a completion script for the shell, on
...
function header(comment, shell, pin) {
  return [
    `${comment} omakit completion for ${shell}. Generated by \`omakit completion ${shell}\` from`,
    `${comment} marketplace pin ${pin}; the categories and tags`,
    `${comment} below are that pin's submission form. Regenerate it by running the command`,
    `${comment} again.`,
  ]
}
...
function bash({ subcommands, categories, tags, pin }) {
  ...
  lines.push(`  local shells=${single(COMPLETION_SHELLS.join(" "))}`)
  ...
  for (const sub of subcommands) {
    ...
    } else if (sub.name === "completion") {
      lines.push('    ((COMP_CWORD == 2)) && COMPREPLY=($(compgen -W "$shells" -- "$cur"))')
    }
    lines.push("    ;;")
  }
  lines.push("  esac")
...
function zsh({ subcommands, categories, tags, pin }) {
  ...
  lines.push(`  shells=(${COMPLETION_SHELLS.map(single).join(" ")})`)
  ...
  for (const sub of subcommands) {
    ...
    if (sub.name === "completion") specs.push(single("1:shell:{compadd -a shells}"))
...
function fish({ subcommands, categories, tags, pin }) {
  ...
  for (const sub of subcommands) {
    ...
    if (sub.name === "completion") lines.push(`complete -c omakit ${when} -a ${single(COMPLETION_SHELLS.join(" "))}`)
