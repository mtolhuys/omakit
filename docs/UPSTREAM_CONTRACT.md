...
| What moved since the pin | `omakit doctor` compares each path in `PIN_PATHS` between the pin and the marketplace's HEAD by tree or blob id (the pin side from the local checkout, the HEAD side through the git-trees API at the exact commit) and lists the ones that differ, with `changedPaths` in `--json`; offline or on any error it says `unknown` |
