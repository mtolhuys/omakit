...
| What moved since the pin | `omakit doctor` compares what omakit reads between the pin and the marketplace's HEAD: each file in the read set by blob id, `registry.json` and `site/catalog.json` by blob id, `.github/ISSUE_TEMPLATE/` by tree id (the pin side from the local checkout, the HEAD side through the git-trees API at the exact commit: three tree reads, a fourth for the `scripts/` tree only when its id moved, a fifth GET for the policy module's text at HEAD only when its blob moved). Graded by what the difference can do to a verdict: `ok` when nothing in the read set moved (the data files are read live; `scripts/` moving elsewhere is said as "in none of the 16 files omakit reads"); `info` when the only moved files are in `WORDING_READS` (`pin.mjs`: the baseline's rendered details, the failure table, one label from each approval script; omakit takes wording or a label out of them and decides nothing by them), the files named as wording only; `advice` when a file omakit executes or reads a rule from moved (the whitelist's complement, 12 of 16, whether or not `securityBaselineVersion` and `securityBaselineEnforcementMode` still read the same at HEAD, since a rule can change without its version), a read file is gone at HEAD, or the form directory moved, with `omakit upgrade` as the action when a newer omakit is published and otherwise that the maintainer is notified, and in both cases that every verdict is the pin's until then (the weekly workflow opens the one issue; doctor never asks for one). The two constants are printed beside every grade. Offline or on any error it says `unknown`. In `--json`, under `evidence`: `changedPaths`, `readLive`, `pinned` as before, plus `reads`, `moved`, `verdictMoved`, `missing` and `policy.{pin,head}`. Measured in `docs/MEASUREMENTS.md` M7: of the three `scripts/` commits since `38060f89`, 5e401552 grades `ok` under this comparison and graded `advice` under the tree comparison; 7dd6e56 and 40315f2 grade `advice` under both. The closure's completeness is held by `tests/unit/pin.test.mjs`: a dynamic `import()`, side-effect import, `require()` or `import.meta.resolve()` in an executed read file fails the suite by file and line (0 of 13 at `b7b29654`). The procedure below is the maintainer's and is not printed |
