...
| `release-unavailable` | none of the newest Omarchy releases has its ISO, `.sha256` and `.sig` all published; or one is published but does not read (a checksum that names no SHA-256 for its ISO, a sidecar over 4 KiB, an ISO whose size is not announced); or the release list named nothing at or above the lab's floor | The message names the release and the object. A 404 means Omarchy has not published all three yet: run it again later. Anything else can be the network between you and iso.omarchy.org (a proxy that rewrites answers): run it again from another network, or pass `--from <omarchy-X.Y.Z.iso>` with its `.sha256` and `.sig` beside it. |
