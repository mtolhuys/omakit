...
one word it closes on is `INSPECTED`. The report is one line per fact and a
second only where something is absent, with a script's lines grouped under
the script; `--full` is every site with every qualifier. `--json` prints
