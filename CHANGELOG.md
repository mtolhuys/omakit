...
- `omakit inspect <plugin-dir>`: what a plugin tree does, as observations.
  ...
  The report is what needs attention, biggest first: one block per review
  class the tree shows, ordered by measured share, with up to five sites
  under each; `--full` is every site with every qualifier. `docs/INSPECT.md` is the contract; `--json`
  is the document.
