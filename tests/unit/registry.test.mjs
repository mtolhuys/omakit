...
test("--offline reads the pin and says so; a HEAD that cannot be read falls back to the pin and never throws", async () => {
  ...
  assert.equal(registrySourceDetail(offline), "registry at the pin b7b29654 (offline)")
  ...
  assert.equal(registrySourceDetail(noHead), "registry at the pin b7b29654; HEAD unreadable: api.github.com did not answer (network-unavailable)")
  ...
  assert.equal(registrySourceDetail(noFile), `registry at the pin b7b29654; HEAD ${HEAD.slice(0, 7)} unreadable: GET ${RAW}/${HEAD}/registry.json returned 503 (github-unavailable)`)
...
test("identity.available judges against HEAD's registry and names it; the pin's figures stay the pin's", async () => {
  ...
  assert.match(baseline.why, /3,619 listed sources/, "the documented figure is the pin's by design")
  ...
  assert.ok(pinned.detail.endsWith("; registry at the pin b7b29654 (offline)"), pinned.detail)
})
