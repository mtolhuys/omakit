...
test("the toolchain is judged by the harness's hash against the pin: patched, unpatched, or unknown", () => {
  ...
  try {
    ...
    assert.match(lines.find(([key]) => key === "on disk")[1], /12,717,359,104 B \(12\.717 GB \/ 11\.844 GiB\)/)
...
test("the command surface: one `lab` entry with four actions in the help, the option table, and tab completion, and the suites offered after run", () => {
  ...
  assert.ok(entry)
  assert.deepEqual([].concat(entry.signature).map((line) => line.split(" ")[2]), ["run", "inspect", "setup", "prune"])
